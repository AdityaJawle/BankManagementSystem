...
public class AccountManagementServiceImpl implements AccountManagementService, UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String crn) throws UsernameNotFoundException {
        ...
        if (user.isPresent()) {
            ...
            // Directly using the User constructor instead of builder
            return new org.springframework.security.core.userdetails.User(
                userObj.getCrn(),
                userObj.getPin(),
                List.of(new SimpleGrantedAuthority("ROLE_" + userObj.getRole()))  // <-- Correct use of SimpleGrantedAuthority
            );
        } else {
    ...
    public Users createUserWithRole(Users user, String role, Users adminUser) {
        ...
        // Prepare admin log using the passed adminUser object (no DB fetch here)
